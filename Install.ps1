...
if($PSVersionTable.PSVersion.Major -lt 3) {
    Write-Warning "SitecoreShipFunctions requires PowerShell 3.0 or better; you have version $($Host.Version)."
    return
}
...
$profileLine = "Import-Module '$installDir\SitecoreShipFunctions.psd1'"
if(Select-String -Path $WhichProfile -Pattern $profileLine -Quiet -SimpleMatch) {
    Write-Host "It seems SitecoreShipFunctions is already installed..."
    return
}

Write-Host "Adding SitecoreShipFunctions to profile..."
@"

# Load SitecoreShipFunctions module
$profileLine

"@ | Out-File $WhichProfile -Append -WhatIf:$WhatIf -Encoding (Get-FileEncoding $WhichProfile)

Write-Host 'SitecoreShipFunctions sucessfully installed!'
Write-Host 'Please reload your profile for the changes to take effect:'
Write-Host "    . $WhichProfile"
Write-Host "To reload, simply type '& `$profile'"
